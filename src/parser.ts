...
const operatorList: Operator[] = [
    { type: '+', priority: 1, operation: (o1: number, o2: number) => { return o1 + o2 } },
    { type: '-', priority: 1, operation: (o1: number, o2: number) => { return o1 - o2 } },
    { type: '*', priority: 2, operation: (o1: number, o2: number) => { return o1 * o2 } },
    { type: '/', priority: 2, operation: (o1: number, o2: number) => { return o1 / o2 } },
    { type: '^', priority: 3, operation: (o1: number, o2: number) => { return Math.pow(o1, o2) } }
]
...
export const sequenceToRPN = (s: string): (Operator | number)[] => {
    const elements = s.split(' ').join('').match(/((?<=(\d|\)))\-(?=(\d|\()))|(\-?\d+(\.\d+)*)|[\+\*\/\(\)\^]/g);
